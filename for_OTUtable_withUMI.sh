...
###########################################################################################
###########################################################################################
# a shell script to generate the OTU table with UMI infomation
###########################################################################################
###########################################################################################

# Usage: bash for_OTUtable_withUMI.sh
# run on my mac pro
...
if [ ! -d for_OTUtable_withUMI ]
then
	mkdir for_OTUtable_withUMI
fi
...
do
	...
	cd ${PCRset}_temp
	echo "${PCRset}" > ${PCRset}_countUMI.txt
	...
	do
  		...
  		then
			...
			sed '1d' ${otuid}_1.txt | sed "s/^.*_//g" | sort -u > ${otuid}_2.txt
			grep -c "^N" ${otuid}_2.txt >> ${PCRset}_countUMI.txt # count UMIs for each OTU in each PCRset
			rm -f ${otuid}*.txt
  		else
			echo "0" >> ${PCRset}_countUMI.txt
			continue
		fi
	done
	
	mv ${PCRset}_countUMI.txt ../for_OTUtable_withUMI/
	cd ../
	rm -rf ${PCRset}_temp

done

paste OTUIDs.txt for_OTUtable_withUMI/* > mock_countUMI_OTUtable.txt
rm -f OTUIDs.txt
rm -rf for_OTUtable_withUMI
